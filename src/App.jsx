...
const BUBBLE_PRODUCTS = [
  {
    ...
    name: "風力發電機風扇",
    desc: "高強度輕量化\n提升轉動效率",
    img: IMAGES.bubbleWind,
  },
  {
    id: 2,
    name: "無人機外殼、風扇",
    desc: "耐衝擊抗疲勞\n延長飛行續航",
    img: IMAGES.bubbleDrone,
  },
  {
    id: 3,
    name: "電動車外殼",
  ...
  },
  {
    ...
    name: "電路基板 PCB",
    desc: "低介電損耗\n確保訊號完整",
    img: IMAGES.bubblePCB,
  },
  {
    id: 5,
    name: "安全頭盔",
    desc: "高剛性防護\n守護工安關鍵",
    img: IMAGES.bubbleHelmet,
  },
];
...
const HomePage = ({ text, setCurrentPage }) => {
  ...
  return (
    <div className="w-full">
      <section
        id="hero-sec"
        className="relative bg-slate-900 h-screen flex items-center text-left text-white overflow-hidden"
      >
      ...
      <section
        id="about-sec"
        className="min-h-screen py-16 px-6 bg-white flex items-center overflow-hidden border-b"
      >
      ...
      <section
        id="bubble-sec"
        className="py-20 min-h-screen bg-slate-50 flex flex-col justify-center overflow-hidden"
      >
...
const ProductsPage = ({ text, activeTab, setActiveTab }) => (
  <div className="w-full">
    <section
      id="prod-main"
      className="min-h-screen py-24 px-6 bg-slate-50 text-slate-900"
    >
...
const QualityPage = ({ text, hoverStepId, setHoverStepId }) => (
  <div className="w-full">
    <section
      id="qual-main"
      className="min-h-screen py-24 px-6 bg-white text-slate-900"
    >
      <div className="container mx-auto">
    ...
    <section
      id="qual-smart"
      className="min-h-screen py-24 px-6 bg-slate-50 flex items-center text-slate-900"
    >
...
const CustomPage = ({ text }) => (
  <div className="w-full">
    <section
      id="cust-main"
      className="min-h-screen py-24 px-6 bg-slate-900 text-white flex items-center overflow-hidden"
    >
      <div className="container mx-auto text-center">
        <h1 className="text-4xl md:text-6xl font-bold mb-6">
          {text.custom.title}
    ...
    <section
      id="cust-lab"
      className="min-h-screen py-24 px-6 bg-white flex items-center overflow-hidden"
    >
...
// ==========================================
// 修改：ContactPage 整合 EmailJS (使用 fetch)
// ==========================================
const ContactPage = ({ text }) => {
  ...
  // Helper Function: 透過 fetch 發送 EmailJS
  ...
  const sendEmail = (e) => {
    ...
    // 您的 EmailJS 設定
    // ==========================================
    const SERVICE_ID = "service_2cf017i";
    const TEMPLATE_ID_ADMIN = "template_77n6tnc"; // 寄給越綾電材的通知信模板
    const TEMPLATE_ID_USER = "template_1tn6uro"; // 寄給客戶的確認信模板
    const PUBLIC_KEY = "glxz3tgPB6GR2JHNd";

    // 準備參數
    const templateParams = {
      from_name: formData.user_name,
      reply_to: formData.user_email,
      to_email: formData.user_email, // 供 TEMPLATE_ID_USER 使用 (請確保後台有設定 {{to_email}})
      message: formData.message,
    };

    // 1. 先寄給公司 (Admin) - 這一步成功就算成功
    sendViaFetch(SERVICE_ID, TEMPLATE_ID_ADMIN, templateParams, PUBLIC_KEY)
      .then(() => {
        console.log("Admin email sent successfully");
        // 只要公司收到，就視為成功，讓 UI 顯示綠勾勾
        setStatus("success");
        setFormData({ user_name: "", user_email: "", message: "" });
        setTimeout(() => setStatus("idle"), 5000);

        // 2. 接著嘗試寄給客戶 (User) - 失敗僅記錄，不影響 UI
        sendViaFetch(SERVICE_ID, TEMPLATE_ID_USER, templateParams, PUBLIC_KEY)
          .then(() => console.log("User confirmation email sent"))
          .catch((err) => {
            console.warn(
              "User confirmation email failed (Check EmailJS template settings)",
              err,
            );
          });
      })
  ...
  return (
    <div className="w-full text-slate-900">
      <section
        id="cont-main"
        className="min-h-screen py-24 px-6 bg-slate-900 flex items-center"
      >
          ...
          <div className="bg-white p-10 rounded-[3rem] shadow-2xl animate-in slide-in-from-right-12 duration-1000">
            ...
            >
              ...
              <button
                ...
                className={`w-full font-extrabold py-3.5 rounded-xl shadow-lg transition-all active:scale-[0.98] mt-2 flex items-center justify-center gap-2 ${
                  status === "success"
              ...
              >
                {status === "idle" && (
                  <>
                    送出技術諮詢申請 <Send size={18} />
                  </>
                )}
                {status === "sending" && (
                  <>
                    發送中... <Loader2 size={18} className="animate-spin" />
                  </>
                )}
                {status === "success" && (
                  <>
                    發送成功！我們會盡快聯繫您 <CheckCircle size={18} />
                  </>
                )}
                {status === "error" && "發送失敗，請稍後再試"}
              </button>
            </form>
          </div>
        </div>
...
const SilkonixSite = () => {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden text-slate-900 bg-white font-sans">
      ...
      <main
        ref={scrollerRef}
        className="flex-grow overflow-y-auto scroll-smooth pt-0 bg-white"
      >
        <div key={currentPage}>
          {currentPage === "home" && (
        ...
        <footer className="bg-slate-50 text-slate-600 py-8 px-6 border-t border-slate-200 flex-shrink-0">
          <div className="container mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
              {/* Brand & Slogan */}
              <div className="md:col-span-1 text-left flex flex-col justify-center">
                <div className="mb-3 flex justify-start">
                  <SilkonixLogo className="h-6 md:h-8" />
                </div>
              ...
              {/* Navigation Links */}
              <div className="md:col-span-3 flex flex-wrap gap-8 md:gap-16 justify-start md:justify-end items-center">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {Object.entries(text.nav).map(([key, label]) => (
                      <button
                        ...
                        className="text-sm text-slate-500 hover:text-blue-600 hover:translate-x-1 transition-all text-left"
                      >
                        {label}
                      </button>
                    ))}
            ...
            <div className="pt-4 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-2">
              <p className="text-[10px] text-slate-400 uppercase tracking-widest font-medium">
                © 2026 {SITE_CONFIG.branding.name_en} Materials Co., Ltd.
              </p>
              <p className="text-[10px] text-slate-400 uppercase tracking-widest font-medium">
